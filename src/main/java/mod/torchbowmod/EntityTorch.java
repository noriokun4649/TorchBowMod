...
public class EntityTorch extends Entity implements IProjectile {
    ...
    private static final DataParameter<Byte> CRITICAL = EntityDataManager.<Byte>createKey(EntityTorch.class, DataSerializers.BYTE);
    private int xTile;
    private int yTile;
    private int zTile;
    ...
    private Block setingBlock;

    /**
     * 1 if the player can pick up the arrow
     */
    public EntityTorch.PickupStatus pickupStatus;
    /**
    ...
    public EntityTorch(World worldIn) {
        super(worldIn);
        this.xTile = -1;
        this.yTile = -1;
        this.zTile = -1;
    ...
    public EntityTorch(World worldIn, EntityLivingBase shooter) {
        ...
        if (entity != null) {
            ...
            if (entity.attackEntityFrom(damagesource, (float) i)) {
                ...
                if (!this.world.isRemote && this.motionX * this.motionX + this.motionY * this.motionY + this.motionZ * this.motionZ < 0.0010000000474974513D) {
                    if (this.pickupStatus == EntityTorch.PickupStatus.ALLOWED) {
            ...
            if (iblockstate.getMaterial() != Material.AIR) {
                ...
                if (!world.isRemote) {
                    if (count) {
                        ...
                        IBlockState torch_state = getSetingBlock().getDefaultState();
                            ...
                            switch (this.getHorizontalFacing()) {
                                case EAST:
                                    ...
                                    if (isBlockAIR(east_pos)) {
                                        world.setBlockState(east_pos, torch_state.withProperty(FACING, this.getHorizontalFacing()));
                                        this.setDead();
                                        break;
                                    }
                                case WEST:
                                    ...
                                    if (isBlockAIR(west_pos)) {
                                        world.setBlockState(west_pos, torch_state.withProperty(FACING, this.getHorizontalFacing()));
                                        this.setDead();
                                        break;
                                    }
                                case NORTH:
                                    ...
                                    if (isBlockAIR(north_pos)) {
                                        world.setBlockState(north_pos, torch_state.withProperty(FACING, SOUTH));
                                        this.setDead();
                                        break;
                                    }
                                case SOUTH:
                                    ...
                                    if (isBlockAIR(south_pos)) {
                                        world.setBlockState(south_pos, torch_state.withProperty(FACING, NORTH));
                                        this.setDead();
                                        break;
                                    }
                                default:
    ...
    private boolean isBlockAIR(BlockPos pos) {
        ...
        Block[] a = {Blocks.AIR, Blocks.TALLGRASS, Blocks.DOUBLE_PLANT, Blocks.RED_FLOWER, Blocks.YELLOW_FLOWER, Blocks.SNOW_LAYER};//空気だとみなすブロックリスト
        for (Block traget : a) {
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("pickup", 99)) {
            this.pickupStatus = EntityTorch.PickupStatus.getByOrdinal(compound.getByte("pickup"));
        } else if (compound.hasKey("player", 99)) {
            this.pickupStatus = compound.getBoolean("player") ? EntityTorch.PickupStatus.ALLOWED : EntityTorch.PickupStatus.DISALLOWED;
        }
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn) {
        if (!this.world.isRemote && this.inGround && this.arrowShake <= 0) {
            boolean flag = this.pickupStatus == EntityTorch.PickupStatus.ALLOWED || this.pickupStatus == EntityTorch.PickupStatus.CREATIVE_ONLY && entityIn.capabilities.isCreativeMode;

            if (this.pickupStatus == EntityTorch.PickupStatus.ALLOWED && !entityIn.inventory.addItemStackToInventory(this.getArrowStack())) {
                flag = false;
            }
    ...
    protected ItemStack getArrowStack() {
        return new ItemStack(getSetingBlock());
    }
    ...
    public enum PickupStatus {
        ...
        public static EntityTorch.PickupStatus getByOrdinal(int ordinal) {
